...
--
-- This script is idempotent: safe to run multiple times against the same
-- database (e.g. after a partial failure, or by accident) without erroring
-- on objects that already exist from a prior run.
...
drop policy if exists "Authenticated view all certificates" on public.certificates;
create policy "Authenticated view all certificates"
  on public.certificates for select to authenticated using (true);

drop policy if exists "Authenticated insert own certificates" on public.certificates;
create policy "Authenticated insert own certificates"
  on public.certificates for insert to authenticated
  with check (auth.uid() = created_by);

drop policy if exists "Owner or admin update certificates" on public.certificates;
...
drop policy if exists "Owner or admin delete certificates" on public.certificates;
...
create unique index if not exists certificates_certificate_no_idx on public.certificates (certificate_no);
